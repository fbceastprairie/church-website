...
-- OR REPLACE makes these safe to re-run.
...
-- Function to CREATE a new user (email, password, role)
-- This bypasses the need to use the Supabase Dashboard
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM auth.users WHERE email = create_user.email) THEN
  ...
  INSERT INTO auth.users (
    ...
    jsonb_build_object('role', role, 'username', split_part(email, '@', 1)),
    now(),
    now(),
    '',
    ''
